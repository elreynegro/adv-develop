    ...
    ['schemaFormProvider', 'schemaFormDecoratorsProvider', 'sfPathProvider',
        function (schemaFormProvider, schemaFormDecoratorsProvider, sfPathProvider) {
            ...
            var uimultiselect = function (name, schema, options) {
                if (schema.type === 'array' && schema.format == 'uimultiselect') {
                    var f = schemaFormProvider.stdFormObj(name, schema, options);
                    f.key = options.path;
    ...
    .directive("toggleModel", function () {
        ...
        return {
            ...
            controller: ['$scope', '$filter', function ($scope, $filter) {
                $scope.$parent.$watch('form.select_models', function () {
                    if ($scope.$parent.form.select_models.length == 0) {
                        $scope.$parent.insideModel = $scope.$parent.$$value$$;
                            if(!$scope.$parent.insideModel.$pristine){
                                $scope.$parent.$error.required;
                            }
                ...
                // for lazy bind
                $scope.$on(BROAD_CAST_NAMESPACE.SCHEMA_UI_SELECT_NGMODEL_PROPAGATION, function (event, savedItems, field) {
                    if (angular.isArray(savedItems)) {
                        if (field === $scope.$parent.form.key[0]) {
                            $scope.$parent.form.select_models = $filter('whereMulti')($scope.$parent.form.schema.items, 'value', savedItems);
                        }
                    }
                });
            }]
