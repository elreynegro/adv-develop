...
class AdvRez_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'advantage-reservations'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'advantage-reservations'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'advantage-reservations'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'advantage-reservations'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'advantage-reservations');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'advantage-reservations'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'advantage-reservations');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'advantage-reservations'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'advantage-reservations') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'advantage-reservations');
            case 'false':
                return __('false', 'advantage-reservations');

            case 'Administrator':
                return __('Administrator', 'advantage-reservations');
            case 'Editor':
                return __('Editor', 'advantage-reservations');
            case 'Author':
                return __('Author', 'advantage-reservations');
            case 'Contributor':
                return __('Contributor', 'advantage-reservations');
            case 'Subscriber':
                return __('Subscriber', 'advantage-reservations');
            case 'Anyone':
                return __('Anyone', 'advantage-reservations');
        }
        return $optionValue;
    }
